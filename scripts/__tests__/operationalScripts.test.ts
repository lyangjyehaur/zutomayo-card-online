...
describe('operational shell scripts', () => {
  ...
  it('keeps the server4 beta deployment backup, Redis, asset, and health gates', () => {
    ...
    expect(deploy).toContain('load_remote_official_release_build_id');
    expect(deploy).toContain('preserving server4 card dataset SHA-256');
    expect(deploy).toContain('preserving server4 official-rulings release build ID');
    ...
    expect(deploy).toContain('--expected-official-build-id');
    expect(smoke).toContain("args.get('expected-official-build-id')");
