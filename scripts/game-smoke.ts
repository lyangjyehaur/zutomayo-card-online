...
import {
  ...
  isValidConstructedDeck,
  validateConstructedDeckIds,
  randomDeck,
  getPresetDeck,
} from '../src/game/cards/deckBuilder';
import {
  advanceChronos,
  confirmReady,
  finishMulligan,
...

{
  ...
  const parsedEffects = new Map<string, ParsedEffect[]>([['2nd_5', [turnEndDamage5, turnStartHeal10]]]);
  ...
  assert.equal(
    G.timingEvents.some((event: TimingEvent) => event.type === 'turnEnd'),
    false,
  );
  assert.equal(
    G.timingEvents.some((event: TimingEvent) => event.type === 'zoneEntered' && event.zone === 'abyss'),
    false,
  );
}

{
  ...
  const parsedEffects = new Map<string, ParsedEffect[]>([['2nd_5', [damageReceivedHeal10]]]);
  resolveTurn(G, parsedEffects);
  assert.equal(G.players[1].hp, 100 - G.lastBattleResult.damage + 10);
  assert.ok(G.timingEvents);
  assert.equal(
    G.timingEvents.some((event: TimingEvent) => event.type === 'damageReceived'),
    false,
  );
}
  ...
  assert.equal(
    G.timingEvents.some(
      (event) =>
        ...
        event.player === 1 &&
        event.zone === 'battleZone' &&
        event.cardDefId === '1st_1',
    ),
    false,
  );
}

{
  ...
  assert.equal(
    G.timingEvents.some(
      (event) =>
        event.type === 'characterReplaced' &&
        event.player === 0 &&
        event.cardDefId === '1st_9' &&
        event.replacedCardDefId === '1st_17',
    ),
    false,
  );
}
...

{
  ...
  assert.deepEqual(parsedCardEffect('2nd_62').action, {
    type: 'addSettableCard',
    params: { count: 1, optional: true },
  });
...

{
  ...
  assert.deepEqual(parseEffect('相手のエリアエンチャントが置かれているなら、すぐにアビスに置く')?.conditions, [
    { type: 'hasAreaEnchant', value: true, target: 'opponent' },
  ]);
  assert.deepEqual(parseEffect('HPが50以下になったなら、パワーチャージャーに置く')?.action, {
    type: 'moveSelfAreaEnchant',
    params: { destination: 'powerCharger' },
  });
  assert.equal(parseEffect('HPが50以下になったなら、パワーチャージャーに置く')?.trigger, 'onDamageReceived');
  assert.deepEqual(
    parseEffect('パワーチャージャーにカードが５枚以上置かれているなら、すぐにアビスに置く')?.conditions,
    [{ type: 'zoneCountComparison', value: 5, operator: 'gte', target: 'powerCharger', owner: 'self' }],
  );
  assert.equal(
    parseEffect('相手のアビスにカードが置かれたとき、すぐにこのカードをアビスに置く')?.trigger,
    'onZoneEntered',
  );
  assert.equal(
    parseEffect('パワーチャージャーにカードを置いたとき、すぐにこのカードをアビスに置く')?.trigger,
    'onZoneEntered',
  );
  assert.deepEqual(parseEffect('バトルに負けたとき、すぐにアビスに置く')?.conditions, [
    { type: 'battleLost', value: true },
  ]);
  assert.deepEqual(
    parseEffect('相手のキャラクターカードの時計を無効にする。昼と夜が入れ替わったターンの終了時にアビスに置く')?.action,
    { type: 'nullifyOpponentClock', params: {} },
  );
  assert.equal(
    parseEffect(
      'パワーチャージャーの電気属性のカード１枚につき、攻撃力+20。相手のHPが30以下になったターンの終了時にアビスに置く',
    )?.trigger,
    'onUse',
  );
  assert.equal(
    parseEffect('相手のキャラクターカードの時計を無効にする。昼と夜が入れ替わったターンの終了時にアビスに置く')
      ?.trigger,
    'onUse',
  );
  ...
  assert.ok(
    areaExpiryEffects.some((effect) => effect.trigger === 'onTurnEnd' && effect.action.type === 'moveSelfAreaEnchant'),
  );
  const chronosExpiryEffects = allParsedEffects.get('2nd_86') ?? [];
  assert.ok(
    chronosExpiryEffects.some(
      (effect) => effect.trigger === 'onChronosChanged' && effect.action.type === 'moveSelfAreaEnchant',
    ),
  );
  const zoneEntryExpiryEffects = allParsedEffects.get('4th_30') ?? [];
  assert.equal(
    zoneEntryExpiryEffects.filter((effect) => effect.rawText.includes('相手のアビスにカードが置かれたとき')).length,
    1,
  );
  ...
  assert.ok(
    returnAreaAndExpireEffects.some(
      (effect) => effect.action.type === 'moveSelfAreaEnchant' && effect.trigger === 'onTurnEnd',
    ),
  );

  const priorityAreaExpiryIds = [
    '2nd_5',
    '2nd_86',
    '2nd_98',
    '3rd_58',
  ...
  for (const id of priorityAreaExpiryIds) {
    ...
    assert.ok(
      allParsedEffects
        .get(id)
        ?.some((effect) => effect.action.type === 'moveSelfAreaEnchant' && effect.trigger === parsed.expiry?.trigger),
      `${id} expiry should be emitted by parseAllEffects`,
    );
  }
  // 官方 QA Q21：條件改用 chronosTimeChanged 檢查事件歷史（含中間轉換）。
  assert.deepEqual(parseEffect(cardEffectText('2nd_86'))?.expiry?.conditions, [
    { type: 'chronosTimeChanged', value: 'nightToDay' },
  ]);
  assert.deepEqual(parseEffect(cardEffectText('2nd_98'))?.expiry?.conditions, [
  ...
  assert.deepEqual(parseEffect(cardEffectText('3rd_91'))?.expiry?.conditions, [
    { type: 'zoneEntered', value: 'abyss', target: 'opponent' },
  ]);
  ...
  assert.ok(
    opponentHealThenDamageEffects.some(
      (effect) => effect.trigger === 'onTurnEnd' && effect.action.type === 'directDamage',
    ),
  );
  ...
  const returnAreaEffect = parseEffect(
    '相手のエリアエンチャントをデッキの底に戻し、以後相手はエリアエンチャントをセットできない。相手がアビスにカードを置いたターンの終了時にパワーチャージャーに置く',
  );
  assert.ok(returnAreaEffect);
  assert.deepEqual(returnAreaEffect.action, {
    type: 'returnAreaEnchantToDeck',
    params: { target: 'opponent', position: 'bottom', lockAreaEnchant: true },
  });
  ...
  const revealedHandView = ZutomayoCard.playerView!({
  ...
  assert.deepEqual(
    handAbyssSwapState.pendingChoice?.options.map((option) => option.id),
    [`hand:${swapHandCard.instanceId}`, `abyss:${swapAbyssCard.instanceId}`],
  );
  assert.equal(
    submitPendingChoice(
      handAbyssSwapState,
      0,
      [`hand:${swapHandCard.instanceId}`, `hand:${swapHandCard.instanceId}`],
      noEffects,
    ),
    false,
  );
  ...
  assert.equal(
    submitPendingChoice(extraEnchantThenDrawState, 0, [extraHandEnchant.instanceId], extraEnchantParsedEffects),
    true,
  );
  ...
  assert.deepEqual(extraEnchantThenDrawState.pendingEffects[0][1]?.effect.action, {
    type: 'drawCards',
    params: { value: 1 },
  });
  ...
  const boostBothByHp = parseEffect(
    'お互いの自分のHPの分だけ攻撃力+。相手のHPが40以下になったターンの終了時にアビスに置く',
  );
  ...
  const setOpponentAttackTo100 = parseEffect(
    'バトルゾーンのカードが（猫リセット）のキャラクターなら、相手の攻撃力を100にする',
  );
  ...
  setOpponentAttackState.players[1].powerCharger = [
    createInstance('1st_9', true),
    createInstance('1st_13', true),
    createInstance('1st_14', true),
  ...
  const setAllClocksToOne = parseEffect(
    'すべてのカードの時計を１にする。ターンの終了時に、相手のフィールドにエリアエンチャントがあるならパワーチャージャーに置く',
  );
  ...
  setAllClocksState.setCardsThisTurn = [
    [setAllClocksState.players[0].setZoneA],
    [setAllClocksState.players[1].setZoneA],
  ];
  ...
  const forceDayAttack = parseEffect(
    'このカードを使用中、自分の攻撃力は常に昼の攻撃力となる。相手がエリアエンチャントを出したターンの終了時にアビスに置く',
  );
  ...
  forceDayAttackState.players[0].powerCharger = [
    createInstance('1st_9', true),
    createInstance('1st_13', true),
    createInstance('1st_14', true),
  ];
  ...
  const opponentDeckTopPowerToPower = parseEffect(
    '相手はターンの開始時にデッキの一番上を公開する。パワーコストが６以上のカードが山札から公開されたらパワーチャージャーに置く',
  );
  assert.equal(opponentDeckTopPowerToPower?.trigger, 'onTurnStart');
  assert.deepEqual(opponentDeckTopPowerToPower?.action, {
    type: 'moveOpponentDeckTopByPowerCost',
    params: { minPowerCost: 6, destination: 'powerCharger' },
  });
  ...
  const useNamedCharacterFromPowerCharger = parseEffect(
    'パワーチャージャーにある（シェードの埃は延長）のキャラクターを１枚選び、このカードの効果として使用する。',
  );
  ...
  assert.equal(
    executeEffect(moveSelfToPower, powerEnteredFromSelfMoveState, 0, {
  ...
  assert.notEqual(
    parseEffect('アビスに炎属性のカードが２枚以上あるなら、時計が真夜中になる')?.action.type,
    'drawCards',
  );
  assert.notEqual(
    parseEffect('パワーチャージャーにカードが５枚以上置かれているなら、すぐにアビスに置く')?.action.type,
    'drawCards',
  );
  const handBottomDraw = parseEffect('手札を１枚選んでデッキの底に置き、カードを１枚引く');
  assert.deepEqual(handBottomDraw?.action, {
    type: 'requestChoice',
    params: { choiceType: 'handToDeckBottomThenDraw', discardCount: 1, drawCount: 1 },
  });
  ...
  assert.deepEqual(executeEffect(abyssElectricDamage, abyssElectricNoopState, 0), {
    success: false,
  ...
  assert.deepEqual(executeEffect(deckTopByPower, deckTopNoopState, 0), {
    success: false,
    message: 'Condition not met',
  });
  ...
  assert.ok(
    deckTopPowerState.timingEvents.some(
      (event) =>
        event.type === 'zoneEntered' &&
        event.player === 0 &&
  ...
  assert.equal(
    executeEffect(deckTopByPower, powerEntryAreaState, 0, {
  ...
  assert.equal(
    executeEffect(opponentDeckTopPowerToPowerEffect, opponentDeckTopPowerEntryState, 0, {
  ...
  assert.deepEqual(
    opponentAbyssReturnState.pendingChoice?.options.map((option) => option.cardInstanceId),
    [returnedAbyssCard.instanceId, untouchedAbyssCard.instanceId],
  );
  ...
  assert.deepEqual(
    opponentAbyssReturnState.players[1].abyss.map((card) => card.instanceId),
    beforeInvalidOpponentAbyss,
  );
  assert.deepEqual(
    opponentAbyssReturnState.players[1].deck.map((card) => card.instanceId),
    beforeInvalidOpponentDeck,
  );
  ...
  assert.deepEqual(
    optionalStudyState.pendingChoice?.options.map((option) => option.cardInstanceId),
    [matchingStudy.instanceId],
  );
  const hiddenOptionalChoiceView = ZutomayoCard.playerView!({
    G: optionalStudyState,
    ctx: {} as Ctx,
    playerID: '1',
  }) as GameState;
  ...
  assert.deepEqual(
    ...
    studyHandBeforeDecline,
  );
  assert.deepEqual(
    optionalStudyState.players[0].deck.map((card) => card.instanceId),
    studyDeckBeforeDecline,
  );
  ...
  assert.deepEqual(
    optionalElementState.pendingChoice?.options.map((option) => option.cardInstanceId),
    [electricPayment.instanceId],
  );
  ...
  assert.deepEqual(
    optionalInvalidState.players[0].hand.map((card) => card.instanceId),
    invalidHandBefore,
  );
  assert.deepEqual(
    optionalInvalidState.players[0].deck.map((card) => card.instanceId),
    invalidDeckBefore,
  );
  ...
  assert.deepEqual(
    optionalNoLegalState.players[0].hand.map((card) => card.instanceId),
    noLegalHandBefore,
  );
  assert.deepEqual(
    optionalNoLegalState.players[0].deck.map((card) => card.instanceId),
    noLegalDeckBefore,
  );
  ...
  assert.equal(
    submitPendingChoice(optionalDeckState, 0, [deckPaymentB.instanceId, deckPaymentA.instanceId], noEffects),
    true,
  );
  assert.deepEqual(
  ...
  assert.deepEqual(
    optionalDeckDeclineState.players[0].hand.map((card) => card.instanceId),
    deckDeclineHandBefore,
  );
  assert.deepEqual(
    optionalDeckDeclineState.players[0].deck.map((card) => card.instanceId),
    deckDeclineDeckBefore,
  );
  ...
  assert.equal(
    submitPendingChoice(optionalDarkState, 0, [darkPaymentA.instanceId, darkPaymentB.instanceId], noEffects),
    true,
  );
  assert.deepEqual(
  ...
  assert.deepEqual(
    optionalDarkInvalidState.players[0].hand.map((card) => card.instanceId),
    invalidSelectedHandBefore,
  );
  assert.deepEqual(
    optionalDarkInvalidState.players[0].deck.map((card) => card.instanceId),
    invalidSelectedDeckBefore,
  );
  ...
  assert.deepEqual(
    optionalDarkNoLegalState.players[0].hand.map((card) => card.instanceId),
    darkNoLegalHandBefore,
  );
  assert.deepEqual(
    optionalDarkNoLegalState.players[0].deck.map((card) => card.instanceId),
    darkNoLegalDeckBefore,
  );
  ...
  assert.deepEqual(
    optionalSelectedOverdrawState.players[0].deck.map((card) => card.instanceId),
    [overdrawRemainingDeck.instanceId],
  );
  assert.deepEqual(
  ...
  const abyssFourPayment = parseEffect(
    'アビスのカードを４枚選び、裏向きにして混ぜ、デッキの底に置く。そうしない場合、ゲームに敗北する。',
  );
  ...
  const opponentPowerCharacterSwap = parseEffect(
    '相手のパワーチャージャーのキャラクターを１枚選び、バトルゾーンのキャラクターと入れ替える。',
  );
  ...
  const fourthSixEffects =
    parseAllEffects(getAllCardDefs().map(({ id, effect }) => ({ id, effect }))).get('4th_6') ?? [];
  ...
  const abyssVariablePayment = parseEffect(
    'アビスのカードを１枚以上選び、裏向きにして混ぜ、デッキの底に置く。そうしない場合、ゲームに敗北する。',
  );
  ...
  const fourth27Effects =
    parseAllEffects(getAllCardDefs().map(({ id, effect }) => ({ id, effect }))).get('4th_27') ?? [];
  assert.deepEqual(fourth27Effects[1]?.action, selectedCountMill.action);

  const abyssSixPayment = parseEffect(
    'アビスのカードを６枚選び、裏向きにして混ぜ、デッキの底に置く。そうしない場合、ゲームに敗北する。',
  );
  ...
  const abyssOneFaceUpPayment = parseEffect(
    'アビスのカードを１枚選び、デッキの底に置く。そうしない場合、ゲームに敗北する。相手のデッキの上から３枚を見て、好きな順番に入れ替えて戻す',
  );
  ...
  const reorderChoice = reorderFollowUpState.pendingChoice as import('../src/game/types').PendingChoice;
  assert.equal(reorderChoice.min, 3);
  assert.equal(reorderChoice.max, 3);
  assert.deepEqual(
    reorderChoice.options.map((option) => option.cardInstanceId),
    [reorderTopA.instanceId, reorderTopB.instanceId, reorderTopC.instanceId],
  );
  const hiddenReorderView = ZutomayoCard.playerView!({
    G: reorderFollowUpState,
    ctx: {} as Ctx,
    playerID: '1',
  }) as GameState;
  ...
  const hiddenChoiceView = ZutomayoCard.playerView!({
    G: fixedAbyssPaymentState,
    ctx: {} as Ctx,
    playerID: '1',
  }) as GameState;
  ...
  assert.deepEqual(
    variableAbyssPaymentState.players[0].abyss.map((card) => card.instanceId),
    [variableRemainingId],
  );
  ...
  const selectedCountIds = [selectedCountAbyssCards[0].instanceId, selectedCountAbyssCards[2].instanceId];
  ...
  assert.deepEqual(
    abyssMoveState.players[1].abyss.map((card) => card.instanceId),
    [abyssOther.instanceId],
  );
  assert.deepEqual(
    abyssMoveState.players[1].deck.map((card) => card.instanceId),
    [bottomBefore.instanceId, abyssCard.instanceId],
  );

  const powerTwoToDeck = parseEffect(
    '相手のパワーチャージャーからSEND TO POWER★★のカードを１枚選び、相手のデッキの底に置く',
  );
  ...
  const powerOneToDeck = parseEffect(
    '相手のパワーチャージャーからSEND TO POWER★１のカードを１枚選び、相手のデッキの底に置く',
  );
  ...
  assert.deepEqual(
    powerMoveState.pendingChoice?.options.map((option) => option.cardInstanceId),
    [legalPower.instanceId],
  );
  ...
  assert.deepEqual(
    powerMoveState.players[1].powerCharger.map((card) => card.instanceId),
    [illegalPower.instanceId],
  );
  assert.deepEqual(
    powerMoveState.players[1].deck.map((card) => card.instanceId),
    [powerBottomBefore.instanceId, legalPower.instanceId],
  );

  const noLegalPowerState = preparedState();
  noLegalPowerState.players[1].powerCharger = [createInstance('1st_9', true)];
  assert.deepEqual(executeEffect(powerTwoToDeck, noLegalPowerState, 0), {
    success: false,
    message: 'No legal cards for choice',
  });
  ...
  assert.deepEqual(
    opponentSwapState.pendingChoice?.options.map((option) => option.cardInstanceId),
    [powerCharacter.instanceId],
  );
  ...
  assert.equal(
    submitPendingChoice(opponentSwapState, 0, [powerCharacter.instanceId, powerCharacter.instanceId], noEffects),
    false,
  );
  ...
  assert.deepEqual(
    opponentSwapState.swappedCardsThisTurn[1].map((card) => card.instanceId),
    [powerCharacter.instanceId],
  );
  assert.ok(opponentSwapState.suppressedEffectCardIdsThisTurn.includes(powerCharacter.instanceId));
  assert.equal(
    opponentSwapState.pendingEffects[1].some((effect) => effect.cardInstanceId === powerCharacter.instanceId),
    false,
  );
  ...
  assert.deepEqual(
    missingBattleSwapState.players[1].powerCharger.map((card) => card.instanceId),
    [missingBattlePower.instanceId],
  );
  ...
  assert.deepEqual(
    noLegalSwapState.players[1].powerCharger.map((card) => card.instanceId),
    [noLegalPower.instanceId],
  );
  ...
  assert.equal(
    collectedSuppressionEffects[0].some((effect) => effect.cardInstanceId === suppressedBattle.instanceId),
    false,
  );
  assert.equal(
    collectedSuppressionEffects[0].some((effect) => effect.cardInstanceId === unsuppressedPlayed.instanceId),
    true,
  );
  ...
  assert.deepEqual(parseEffect('アビスに炎属性のカードが２枚以上あるなら、時計が真夜中になる')?.action, {
    type: 'clockSet',
    params: { value: 0 },
  });
  ...
  assert.deepEqual(clockRange.action, {
    type: 'requestChoice',
    params: { choiceType: 'clockAdvance', min: 0, max: 5 },
  });
  ...
  assert.deepEqual(
    clockRangeState.pendingChoice?.options.map((option) => option.id),
    ['advance-0', 'advance-1', 'advance-2', 'advance-3', 'advance-4', 'advance-5'],
  );
  ...
  assert.deepEqual(areaBottom.action, {
    type: 'returnAreaEnchantToDeck',
    params: { target: 'opponent', position: 'bottom' },
  });

  const areaTop = parseEffect('相手のエリアエンチャントを相手のデッキの上に置く');
  assert.ok(areaTop);
  assert.deepEqual(areaTop.action, {
    type: 'returnAreaEnchantToDeck',
    params: { target: 'opponent', position: 'top' },
  });

  const areaCardTop = parseEffect('相手のエリアエンチャントカードを相手のデッキの上に置く');
  assert.ok(areaCardTop);
  assert.deepEqual(areaCardTop.action, {
    type: 'returnAreaEnchantToDeck',
    params: { target: 'opponent', position: 'top' },
  });

  assert.deepEqual(parsedCardEffect('1st_104').action, {
    type: 'millDeckToAbyss',
    params: { target: 'opponent', count: 1 },
  });
  ...
  assert.deepEqual(parsedCardEffect('3rd_21').action, {
    type: 'returnAreaEnchantToDeck',
    params: { target: 'opponent', position: 'top' },
  });
  ...
  assert.deepEqual(realAreaTop.action, {
    type: 'returnAreaEnchantToDeck',
    params: { target: 'opponent', position: 'top' },
  });
}
...

{
    ...
    effects.some(
      (effect) =>
        ...
        effect.conditions.some(
          (condition) =>
            condition.type === 'zoneEntered' && condition.value === 'abyss' && condition.target === 'opponent',
        ),
    ),
  );
  const G = preparedAreaEnchantState('4th_30', 0);
  G.players[0].powerCharger = [createInstance('1st_13', true), createInstance('1st_13', true)];
  ...
  assert.equal(
    executeEffect(sendToAbyss, sendState, 0, {
  ...
  assert.equal(
    executeEffect(millToAbyss, millState, 0, {
...

{
  ...
  assert.deepEqual(
    G.pendingChoice?.options.map((option) => option.cardInstanceId),
    [noEffectCopy.instanceId, legalCopy.instanceId],
  );
  const parsedEffects = parseAllEffects(getAllCardDefs().map(({ id, effect }) => ({ id, effect })));
...

{
  ...
  assert.deepEqual(
    G.players[1].abyss.map((card) => card.instanceId),
    [top.instanceId, second.instanceId],
  );
  assert.deepEqual(
    G.players[1].deck.map((card) => card.instanceId),
    [third.instanceId],
...

{
  ...
  assert.deepEqual(parsedCardEffect('1st_58').conditions, [{ type: 'opponentPowerCost', value: 2, operator: 'gte' }]);
  ...
  assert.deepEqual(parsedCardEffect('4th_29').conditions, [{ type: 'noCardInAbyss', value: true, owner: 'opponent' }]);
  assert.deepEqual(parsedCardEffect('1st_61').trigger, 'onChronosChanged');
  assert.deepEqual(parsedCardEffect('1st_61').conditions, [{ type: 'chronosTimeChanged', value: 'dayToNight' }]);
  ...
  assert.deepEqual(
    parseEffect('このカードの効果でカードを引いたなら、手札の数はゲーム終了まで１枚増える')?.conditions,
  ...
  assert.deepEqual(parsedCardEffect('4th_8').conditions, [{ type: 'handElements', value: 4 }]);
  ...
  assert.deepEqual(parsedCardEffect('4th_34').conditions, [{ type: 'opponentAttack', value: 0, operator: 'eq' }]);
  assert.deepEqual(parsedCardEffect('4th_60').conditions, [{ type: 'opponentSendToPower', value: 2, operator: 'eq' }]);
  assert.deepEqual(parsedCardEffect('3rd_1').conditions, [{ type: 'chronosPosition', value: 'midnight' }]);
  ...
  assert.equal(
    executeEffect(parsedCardEffect('3rd_103'), revealPowerEntryState, 0, {
  ...
  assert.ok(
    parsedEffects
      .get('2nd_84')
      ?.some((effect) => effect.action.type === 'setOpponentElement' && effect.action.params.value === '電気'),
  );
  ...
  assert.deepEqual(
    useFromAbyssState.pendingChoice?.options.map((option) => option.cardDefId),
    ['1st_92'],
  );
  ...
  assert.deepEqual(
    handElementsState.revealedHandCardIds[0],
    handElementsState.players[0].hand.map((card) => card.instanceId),
  );
  ...
  extraChaosElementsState.players[0].abyss = ['1st_1', '1st_2', '1st_3', '1st_4', '4th_6'].map((id) =>
    createInstance(id, true),
  );
  ...
  const elementOverrideEffect = parsedEffects
    .get('2nd_84')
    ?.find((effect) => effect.action.type === 'setOpponentElement');
  ...
  assert.deepEqual(
    enchantOnlyPending[1].map((effect) => effect.cardDefId),
    ['1st_1'],
  );
  ...
  namedReducedCostState.players[0].powerCharger = [
    createInstance('1st_9', true),
    createInstance('1st_10', true),
    createInstance('1st_17', true),
  ];
  ...
  abyssBoostState.players[0].abyss = [
    createInstance('1st_9', true),
    createInstance('1st_10', true),
    createInstance('1st_13', true),
  ];
  ...
  assert.deepEqual(executeEffect(parsedCardEffect('1st_58'), opponentPowerCostState, 0), {
    success: false,
    message: 'Condition not met',
  });
  ...
  assert.deepEqual(executeEffect(parsedCardEffect('2nd_14'), abyssElementCountState, 0), {
    success: false,
    message: 'Condition not met',
  });
  ...
  powerChargerCountState.players[0].powerCharger = [
    createInstance('1st_9', true),
    createInstance('1st_10', true),
    createInstance('1st_13', true),
    createInstance('1st_17', true),
  ];
  assert.deepEqual(executeEffect(parsedCardEffect('3rd_56'), powerChargerCountState, 0), {
    success: false,
    message: 'Condition not met',
  });
  ...
  assert.deepEqual(executeEffect(parsedCardEffect('3rd_87'), opponentHpExactState, 0), {
    success: false,
    message: 'Condition not met',
  });
  ...
  assert.deepEqual(executeEffect(parsedCardEffect('4th_29'), noOpponentAbyssState, 0), {
    success: false,
    message: 'Condition not met',
  });
  ...
  perCountBoostState.players[0].abyss = [
    createInstance('1st_4', true),
    createInstance('1st_21', true),
    createInstance('1st_9', true),
  ];
  ...
  assert.deepEqual(
    new Set(revealBoostState.revealedHandCardIds[0]),
    new Set([taidadaA.instanceId, taidadaB.instanceId]),
  );
  ...
  characterPowerExpiryState.players[0].powerCharger = [
    createInstance('1st_9', true),
    createInstance('1st_10', true),
    createInstance('1st_17', true),
  ];
  characterPowerExpiryState.timingEvents.push({
    type: 'zoneEntered',
    player: 0,
    zone: 'powerCharger',
    cardDefId: '1st_9',
