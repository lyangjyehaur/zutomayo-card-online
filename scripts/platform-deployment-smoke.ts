...
function resolveConfig(): SmokeConfig {
  ...
  return { httpUrl, wsUrl, timeoutMs };
}
...
function normalizeSeatReservation(response: FlatSeatReservation | SeatReservation): SeatReservation {
...
async function assertLobbyJoin(config: SmokeConfig): Promise<void> {
  ...
  await suppressExpectedColyseusClientWarnings(async () => {
    const room = await withTimeout(
      client.consumeSeatReservation(normalizeSeatReservation(response.data as FlatSeatReservation)),
      config.timeoutMs,
