...
0.2.4 では、カード図鑑、デッキ共有メタデータと相性提案、第 4 弾 SE 105～107 の効果、Refine 管理画面を追加しました。Chronos／HP／攻撃の解決演出、Online／AI ロビー、公開ルーム一覧、対戦チャット、同じルームでの再戦も再設計し、チュートリアルは正式対戦と同じ解決表示を利用します。

### ゲームと対戦

- ローカル 2 人対戦と、かんたん／ふつう／むずかしい AI。難しい AI は現在、カード効果を含まない当該ターンの lookahead を使用します。制約と改修方針は [AI 難易度ロードマップ](docs/ai-difficulty-roadmap.md) を参照してください。
- 4 パック・425 枚のカードデータと、270 行すべての効果テキスト解析。個別にレビュー・監査して公開する第 4 弾 SE 105～107 を含みます。
