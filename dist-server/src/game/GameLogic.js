...
exports.getChronosTime = getChronosTime;
exports.getPriorityPlayer = getPriorityPlayer;
exports.getPlayerPowerExport = getPlayerPowerExport;
exports.getEffectiveAttack = getEffectiveAttack;
exports.setupGame = setupGame;
exports.resolveJanken = resolveJanken;
exports.mulligan = mulligan;
exports.isSetupComplete = isSetupComplete;
exports.selectCard = selectCard;
exports.getMaxSetCards = getMaxSetCards;
exports.revealCards = revealCards;
exports.advanceChronos = advanceChronos;
exports.swapCards = swapCards;
exports.resolveBattle = resolveBattle;
exports.endTurn = endTurn;
exports.checkGameEnd = checkGameEnd;
const loader_1 = require("./cards/loader");
const deckBuilder_1 = require("./cards/deckBuilder");
// ===== Helpers =====
...
function sendToZone(card, player) {
...
function getPlayerPower(player) {
    return player.powerCharger.reduce((sum, c) => {
        const def = (0, loader_1.getCardDef)(c.defId);
        return sum + (def?.sendToPower || 0);
    }, 0);
}
function getChronosTime(G) {
    const pos = G.chronos.position % 12;
    return pos < 6 ? 'night' : 'day';
}
function getPriorityPlayer(G) {
    const time = getChronosTime(G);
    return time === 'night' ? G.chronos.nightSidePlayer : (1 - G.chronos.nightSidePlayer);
}
function getPlayerPowerExport(player) {
    return getPlayerPower(player);
}
function getEffectiveAttack(card, G, playerIdx) {
    const def = (0, loader_1.getCardDef)(card.defId);
    if (!def || !def.attack)
        return 0;
    const time = getChronosTime(G);
...
// ===== Setup Phase =====
function setupGame(deck0Name, deck1Name) {
    const deck0 = (0, deckBuilder_1.shuffleDeck)(deck0Name ? (0, deckBuilder_1.getPresetDeck)(deck0Name) : (0, deckBuilder_1.randomDeck)());
    const deck1 = (0, deckBuilder_1.shuffleDeck)(deck1Name ? (0, deckBuilder_1.getPresetDeck)(deck1Name) : (0, deckBuilder_1.randomDeck)());
    const makePlayer = () => ({
        hp: 100,
        deck: [],
        hand: [],
...
function resolveJanken(G, choice0, choice1) {
    const beats = {
        rock: 'scissors',
        paper: 'rock',
        scissors: 'paper',
...
// ===== Mulligan =====
function mulligan(G, playerIdx, indicesToRedraw) {
    if (G.mulliganUsed?.[playerIdx])
        return;
...
function selectCard(G, playerIdx, handIndex, slot) {
    ...
    G.setCardsThisTurn[`player${playerIdx}`].push(card);
    return true;
}
function getMaxSetCards(G, playerIdx) {
    if (G.turn === 0)
        return 1;
...
function revealCards(G) {
    ...
    for (const player of G.players) {
        if (player.setZoneA)
            player.setZoneA.faceUp = true;
        if (player.setZoneB)
            player.setZoneB.faceUp = true;
    }
    // Non-character cards go to power charger/abyss immediately
    for (const player of G.players) {
        for (const slot of ['setZoneA', 'setZoneB']) {
...
// ===== Phase: Time =====
function advanceChronos(G) {
    let totalClock = 0;
    for (const playerIdx of [0, 1]) {
        const key = `player${playerIdx}`;
        for (const card of G.setCardsThisTurn[key]) {
            const def = (0, loader_1.getCardDef)(card.defId);
            if (def)
                totalClock += def.clock;
        }
    }
...
function swapCards(G) {
    for (const playerIdx of [0, 1]) {
        ...
        if (player.setZoneA) {
            ...
            if (oldBattle)
                sendToZone(oldBattle, player);
        }
        if (player.setZoneB) {
            const def = (0, loader_1.getCardDef)(player.setZoneB.defId);
            if (def?.type === 'Area Enchant') {
                const oldC = player.setZoneC;
                player.setZoneC = player.setZoneB;
                player.setZoneB = null;
                if (oldC)
                    sendToZone(oldC, player);
            }
        }
    }
...
function checkGameEnd(G) {
    ...
    if (G.players[0].deck.length === 0 && G.players[0].hand.length === 0)
        return 'Player 1 wins! (deck empty)';
    if (G.players[1].deck.length === 0 && G.players[1].hand.length === 0)
        return 'Player 0 wins! (deck empty)';
    return null;
}
