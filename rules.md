...
> 來源: zutomayocard.net/start-guide + ルールガイド Ver.1.0.0 (2026/2/16) + グランドルール Ver.1.0.0 (2026/7/21)
...
## Grand Rules (グランドルール v1.0.0)

- 卡牌文字與通則衝突時，以卡牌文字優先；無法完整執行時盡可能執行可行部分。
- 「不能」效果優先於要求執行的指示；重複套用既有狀態不算再次執行。
- 同時選擇由選擇開始當下的優先玩家先行；公開資訊的「最多 N 張」須選 1 至 N，非公開資訊可選 0 至 N。
- 優先玩家開始效果結算後，即使效果令 Chronos 的晝夜改變，也要完成該玩家全部效果後才輪到對手。
- 效果要求抽取或從牌組移動指定張數而牌組不足時，無法完成的玩家立即敗北。
- 雙方均無法停止的永久循環判為平手。
