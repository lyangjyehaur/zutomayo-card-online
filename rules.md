...
1. **卡牌設定**: 每位玩家至少設置 1 張；勝者／平手最多 1 張，敗者最多 2 張
2. **公開**: 同時翻開
3. **時間推進**: 合計時計推 medal
...
以某張卡的效果使用另一張卡的效果時，只檢查實際發動來源卡的 Power Cost，不另行檢查被複製卡的 Power Cost。
...
站內 `/rules/grand` 依 2026 年 7 月 21 日官方 PDF 收錄完整日文條文及五語逐條翻譯，不以章節摘要代替正文。資料保留官方原始編號，包括缺少 `3.9` 及兩個不同標題均編為 `10.2` 的情況。
...
- 手牌或牌組卡牌因效果公開後，於效果處理結束時恢復為隱藏狀態；進入公開區域的卡牌維持公開。
- 同一回合內，自動效果鏈重現完全相同狀態且沒有任何玩家選擇可中斷時，視為雙方均無法停止的永久循環並判為平手；可由玩家改變行動的循環不會自動判和。
