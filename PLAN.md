...
1. **2 張勘誤卡**（1st_2, 1st_5）的 effect 字段是原始文字，需要查官方 errata 頁面取得修正後的文字
2. **14 張含 HTML 的效果** — 已在 cards.json 中清理，但 errata URL 連結丟失
3. **Area Enchant 跨回合邏輯** — 持續效果的觸發時機需要仔細對照 Q&A
4. **卡圖版權** — 官方允許非商用下載，可直接使用
5. **新卡包同步** — Meilisearch API 可重複拉取，需要定期更新
