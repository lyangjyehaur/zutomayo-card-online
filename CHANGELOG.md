...
- **管理員通知渠道**：可從管理後台即時啟停 Bark、Telegram 與自訂 Webhook，憑證加密保存並支援同時分派；新聯絡郵件會通知所有啟用渠道，單一渠道失敗不阻塞其他渠道或收件。

- **連線生命週期分析**：以不含身份資訊的座位、遊戲階段、開局後秒數與斷線秒數保存 bounded disconnect/reconnect 事件，並提供分 mode／step／seat 的重連時長彙總。

### Changed

- **聯絡信箱寄件者**：管理後台回覆的顯示名稱改為無空格的 `ZTMYCardOnline`，避免郵件客戶端截斷後被誤認為官方服務。

### Fixed

- **對戰分析可靠性**：trusted direct 對戰不再誤標缺少來源，分析成功計數只在 PostgreSQL transaction COMMIT 後增加，避免 rollback 被誤報為永久保存。
- **手機效果選牌**：手機點擊「待選卡牌」時不再因焦點事件先開啟詳情而中斷選取；卡牌點擊負責選取，資訊按鈕仍可獨立開啟詳情。
...
- **實體卡收藏**：圖鑑可標記與篩選已持有實體卡；未登入時保存在本機，登入後合併至帳號並支援跨裝置同步及帳號資料匯出。
- **圖鑑篩選**：新增稀有度與持有狀態篩選，關鍵字搜尋與牌組編輯器一致，可搜尋全部支援語言的卡名、歌名、效果與卡號。

### Changed

- **勘誤匯入安全性**：卡牌 seed／官方文本匯入不再刪除並重建勘誤表；來源欄位不一致時改為中止並要求使用正式官方裁定同步流程。
- **增量卡發布流程**：reviewed-unlisted manifest v2 支援候選 ID 到正式卡牌 ID 映射、可遊玩／僅展示雙軌驗證與完整衍生語言；沒有印刷屬性或成本的展示卡不再被迫填入虛構資料。
- **限定卡圖處理**：PNG 審核原檔另產生等比例 JPEG 發布檔，非標準寬高比不裁切或拉伸；圖鑑卡位改以完整顯示卡面為優先。
- **增量卡翻譯門檻**：所有已發布卡牌（包含僅展示卡）必須具備完整四語文本，且效果翻譯必須通過共用標準術語字典才能發布。
...
- **展示卡圖鑑內容**：修復僅展示卡圖片、多語名稱與效果無法顯示，以及圖鑑顯示不存在的空發行分類；屬性篩選與卡牌標籤改用目前介面語言。
- **第四彈術語**：`4th_106` 的中文效果改用標準 `Chronos` 名稱，不再顯示「克洛諾斯」。

## [0.2.4] - 2026-07-28

卡牌資料、對戰呈現與大廳流程更新：整合牌組分享中繼資料、卡牌圖鑑與搭配推薦，加入第四彈 SE 卡牌效果與 Refine 管理後台，並完成新版戰場結算、Online／AI 大廳、聊天及同房再戰流程。

### Added

- **卡牌圖鑑與推薦**：新增公開卡牌圖鑑、可分享詳情頁、限定／展示用卡狀態，以及以卡名、歌曲、屬性、卡種與系列解釋推薦原因的搭配建議。
- **第四彈 SE 卡牌效果**：補上第四彈 105～107 的卡牌效果解析、選擇流程、執行邏輯與規則測試。
- **第四彈 SE 卡牌發布**：新增固定 allowlist、來源雜湊、四語文本、交易式 PostgreSQL upsert、發布後驗證與管理 audit log，正式卡池更新為 425 張、270 行效果。
- **Refine 管理後台**：以 Refine 5 headless、React Router 7 與既有設計系統重建 `/admin/*`；新增卡牌列表／新增／編輯 resource 頁，並整合歌曲、官方裁定、使用者、對戰、聊天、牌組分享、營運、About、公告、翻譯服務及介面翻譯稽核。
- **公開房間列表**：Online 大廳可瀏覽並加入公開自訂房，並保留房間代碼與牌組選擇作為主要進房流程。
...
- **戰場結算呈現**：以戰場內逐卡數字與 Chronos 徽章推進取代遮擋時針的結算彈窗，並依序呈現攻擊、減傷、最終傷害、HP 恢復及直接操作 Chronos 的卡牌效果。
- **Online／AI 大廳**：重整桌面與手機版資訊層級、牌組選擇及房間操作；AI 對手可選本機或帳號自訂牌組，預設仍使用隨機牌組。
- **教學結算流程**：新手教學改用正式戰場共用的 Chronos、HP 與攻擊結算元件，並修正教學結束後的導航。
- **CI 分流**：一般 push 執行核心瀏覽器 E2E；手動發布流程才執行完整 Playwright、PWA 與發布閘門，兼顧回饋速度與部署覆蓋。

### Fixed

- **Grand Rules 8.2.2**：效果要求從牌組移動指定張數至 Abyss、但牌組不足時，改為處理現有卡牌後立即判定該牌組持有者敗北，不再沿用待修正的 Q70 舊裁定延後至回合結束。
- **Q4 超時出牌**：回合超時且尚未設置卡牌時，由伺服器自動設置一張合法卡再確認；沒有合法卡可設置時明確判負，不再允許零張牌跳過。
- **Q48 複製效果**：從 Abyss／Power Charger 複製卡牌效果時，Power Cost 與效果禁用改為檢查實際發動來源，不再以被複製卡的 Power Cost 錯誤移除效果。
- **Grand Rules 10.3／10.4**：牌組頂與手牌因效果公開後會恢復隱藏，移入牌組的卡牌一律蓋回；公開內容另寫入對戰記錄供雙方核對。
- **Grand Rules 10.1**：自動效果鏈於同一回合重現完全相同的規則狀態，且沒有玩家選擇可中斷時，依永久循環規則立即判定平手；結算頁會顯示對應原因。
- **線上對局聊天**：修復對局訊息未可靠同步至對手的問題，並加入未讀提示與目前對話的已讀處理。
- **同房再戰與離房**：對局結束後可由原房間建立雙方一致的新對局，修復新建房間後退出無法返回大廳及舊 session／座位憑證殘留問題。
- **Grand Rules 完整條文與章節結構**：以官方 PDF 全文重建 45 個章節節點及 190 項以上編號條文，五語譯文逐項保留相同條文編號；閱讀頁不再以 11 段章節摘要冒充完整翻譯，也不再合併第 2／3 章及第 6 至 9 章。發布閘門會檢查原文長度、編號覆蓋、各語條文對齊與官方第 1 至 10 章。
- **Floor Rules 全文與賽事程序**：以官方 14 頁 PDF 重建 10 個主章及 27 個子節，完整收錄參賽禮儀、角色責任、對戰用品、洗牌、BO3、退賽、結果操作與五級處分，並完成繁中、簡中、粵語、英語及韓語全文翻譯；發布閘門會拒絕缺章、摘要式來源、短譯文及遺漏清單／步驟的資料。
