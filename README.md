...
- Server-validated choice flows cover hand-to-deck-bottom draw, focused card moves, Clock choices, own Abyss-to-deck-bottom payments that lose if unpaid, the `4th_27` selected-count mill follow-up, and the `4th_6` opponent Power Charger Character <-> Battle Zone Character swap with swapped-in effect suppression.
