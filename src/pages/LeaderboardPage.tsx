  ...
  return (
    <ScrollPageLayout>
      ...
      {loading && (
        ...
        {error && (
          <Panel className="border-l-2 border-accent-action/50 bg-accent-action/10 text-xs text-accent-action/80" role="alert">
            {error}
          </Panel>
        )}
          ...
          <>
            <Panel className="hidden overflow-x-auto lg:block" size="lg">
              <DataListTable>
                ...
                <tbody>
                  {entries.map((entry, i) => (
                    <tr key={entry.id} className={entry.id === currentUserId ? 'bg-accent-primary/10' : 'odd:bg-surface-canvas/30'}>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:hidden">
              {entries.map((entry, i) => (
                <Card key={entry.id} as="article" className={entry.id === currentUserId ? 'ring-accent-primary/50' : ''}>
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="shrink-0 text-right">
                      <span className="font-mono text-minutia uppercase tracking-[var(--tracking-control)] text-content-primary/40">ELO</span>
