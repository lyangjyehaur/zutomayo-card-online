...
function handleHeal({ effect, G, player, context, me, opponent, opponentIndex, value }: EffectHandlerArgs): {
  ...
  if (effect.action.params.target === 'opponent') {
    ...
    const breakdown = buildEffectHpChangeBreakdown(
      ...
      [
        {
          label: 'board.hpChange.source',
          value: sourceCardDefId ?? '—',
...
function handleRequestChoiceEffect(args: EffectHandlerArgs): { success: boolean; message: string } {
  return handleRequestChoice({
    effect: args.effect,
    G: args.G,
    player: args.player,
    me: args.me,
    opponent: args.opponent,
    opponentIndex: args.opponentIndex,
    power,
...
const effectHandlers: Record<ActionType, EffectHandler> = {
  ...
  requestChoice: handleRequestChoiceEffect,
