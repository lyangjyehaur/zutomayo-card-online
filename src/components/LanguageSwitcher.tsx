...
export function LanguageSwitcher() {
  ...
  return (
    <label className="inline-flex shrink-0 items-center gap-1.5" title={`${t('settings.language')}: ${getLocaleLabel(locale)}`}>
      <span className="hidden text-[10px] uppercase tracking-[0.3em] text-bone/40 sm:inline">{t('settings.language')}</span>
      <select
