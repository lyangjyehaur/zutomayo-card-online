/**
 * Store official card errata separately from historical printed card text.
 *
 * The flags on `cards` support common filtering without a join. Full source,
 * correction, and translation-review data lives in `card_official_errata`.
 */
...
export const down = (pgm) => {
  pgm.dropIndex('cards', ['has_official_errata'], {
    ifExists: true,
    name: 'idx_cards_has_official_errata',
  });
  pgm.dropTable('card_official_errata', { ifExists: true, cascade: true });
  pgm.dropColumn('cards', 'official_errata_url', { ifExists: true });
  pgm.dropColumn('cards', 'official_errata_affects_effect', { ifExists: true });
  pgm.dropColumn('cards', 'official_errata_affects_name', { ifExists: true });
  pgm.dropColumn('cards', 'official_errata_id', { ifExists: true });
  pgm.dropColumn('cards', 'has_official_errata', { ifExists: true });
};
