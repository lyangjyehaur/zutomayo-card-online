...
| `APP_BUILT_AT`        | build/start time                    | ISO 8601 UTC build time shared by all services in one release. The lobby and PWA update UI convert it to the browser's local time in the same compact release label.                                      |
...
| `VITE_APP_BUILT_AT`               | `APP_BUILT_AT`       | Must use the release-wide UTC build time; the lobby, update prompt, and completion toast display it in the browser's local time.                                          |
...
| `APP_BUILT_AT`                          | process start time                  | ISO 8601 UTC build time returned by `/api/app-version`; deployments must keep it aligned with the frontend bundle.                                                                                             |
...
| `APP_BUILT_AT`                     | process start time                        | ISO 8601 UTC build time; release tooling keeps it aligned with `game` and `api`.                                                                                                                                         |
...
並寫入 SHA-256 → checkout `origin/master` → 同步 `APP_BUILD_ID`、`APP_BUILT_AT`、`APP_VERSION`、
