...
- Current merge tree 的 `npm run verify` 通過：Vitest 167 個 test files、1443 tests、dependency patch、資料來源政策、release/operational config、coverage gate、i18n 與 production/PWA build。前一個 pushed hardening commit `ecb442ad` 的 GitHub CI run `29633548114` 四個 jobs 全部通過，E2E 40 passed／1 skipped、無 retry/flaky；本次尚未 push 的 action runtime 變更仍須通過相同 remote CI 才能視為最終自動化證據。
...
- CI／CD／browser matrix 的 `actions/checkout` 與 `actions/setup-node` 已由 retired Node 20 runtime 升到官方 `v7.0.0` Node 24 runtime，仍以 verified full commit SHA pin 住；release config 新增 exact core-action commit 契約，mutable tag、舊 runtime SHA 或任意其他 pinned commit 都會 fail closed。本機完整 verify 已通過，仍待本次 remote CI 自我驗證。
...
- [x] `npm run verify`：2026-07-18 current merge tree 通過，Vitest 167 test files / 1443 tests / production-PWA build；前一個 pushed hardening commit `ecb442ad` 的 GitHub CI run `29633548114` 四個 jobs 全部通過，本次 action runtime 變更待 remote CI。
