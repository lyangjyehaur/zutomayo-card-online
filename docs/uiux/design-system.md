...
| `CompactCard` | `CardView` 的 `sm` / `md` 尺寸：卡圖，數值以原卡面為準       |
| `FullCard`    | `CardView size="xl"`：詳情用，含完整數值                     |
| `CardStack`   | 牌組/深淵/充能堆疊：錯位縮圖 ≤3 張＋計數＋（可選）點擊開摘要 |
| `CardSlot`    | 空/占用槽位：label 水印、佔位虛框、undo 提示                 |

`CardCostTag` 僅可作為 `CardView` 的外部兄弟元素，用於我方手牌、重抽與選卡畫面的原始 Power Cost 快覽；不得放進卡面或遮住原卡時計／數值。Power 不足可用 danger 狀態提示，但顯示值仍為卡面原始 cost，臨時減費只影響狀態判定。
...
| **PlayerStatus**                  | 雙方名+HP+計數           | 單軌 10 刻度 HP bar | healthy·warning·danger；連續填充保留精確 HP，十格刻度供快速估算；戰鬥時由 BattleResolutionLayer 顯示數字倒數與延遲傷害殘影 | —                       | —                                   | `aria-live` HP 變化              | 遮擋戰場             |
| **BattleResolutionLayer**         | 場上攻擊力比較與 HP 結算 | fixed overlay       | compare·equation·strike·damage                                                                                             | 非阻塞，自動依序播放    | 時計動畫完成後開始                  | `role=status aria-live=polite`   | 不可遮住互動操作     |
| **ChronosResolutionLayer**        | 時計合計與卡牌效果推移   | fixed overlay       | contribution·advance·rewind·set                                                                                            | 非阻塞，自動依序播放    | 依來源卡與錶盤刻度定位              | `role=status aria-live=polite`   | 不可顯示虛構時刻數字 |
| **ResolutionTimeline**            | 一般對戰結算播放協調     | 無視覺容器          | chronos·battle·general                                                                                                     | 依 notice ID 自動播放   | 同時只啟用一個 renderer             | 繼承目前 renderer                | 不可跨事件插隊       |
