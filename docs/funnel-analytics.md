...
| `F_Match_Connection_Attempt` | Initial connection or reconnect attempt began           | `match_mode`, `connection_phase`, `elapsed_s` |
| `F_Match_Connection_Success` | Initial connection or reconnect succeeded               | `match_mode`, `connection_phase`, `elapsed_s` |
...
3. Connection: `Connection_Attempt -> Connection_Success`, grouped by `connection_phase`; calculate success ratio and successful `elapsed_s` p95/p99 separately for initial and reconnect attempts.
4. First match: `Match_Start -> Match_Complete`, grouped by viewport, locale, and outcome.
5. Activation: first entry pageview or `entry_route -> Tutorial_Complete/Queue_Match -> Match_Complete -> First_Win`.
...
For LS-09, export only aggregate counts, timing percentiles, and access-controlled query receipt URLs into the player/product evidence packet. Validate the packet with `npm run evidence:player-product`; the schema, observation privacy boundary, and completion criteria are recorded in [`PLAYER_PRODUCT_EVIDENCE_BASELINE.md`](PLAYER_PRODUCT_EVIDENCE_BASELINE.md).
