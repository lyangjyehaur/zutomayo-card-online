...
| hard   | 枚舉完整卡牌 + A/B 卡位計畫，以正式效果與選擇規則模擬完整當前回合，加入有限下回合價值並受 300 ms 預算限制 |
...
目前未知對手卡以中性未知狀態評估，尚未實作依公開牌組組成的 seeded sampling 或 Web Worker。完整進度與限制見 [AI 難度現況與改造路線](ai-difficulty-roadmap.md)。
