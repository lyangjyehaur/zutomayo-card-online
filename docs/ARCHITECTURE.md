...
ZUTOMAYO CARD Online 的系統架構文檔。本文以三層架構為主軸，說明前端 SPA、boardgame.io 遊戲伺服器、獨立 API 伺服器之間的職責劃分與互動方式，並涵蓋遊戲邏輯、資料層、線上對戰流程、可觀測性與部署。
...
系統由三層組成：前端 SPA、boardgame.io 遊戲伺服器（game）、獨立 API 伺服器（api）。game 與 api 共用同一個 PostgreSQL 與 Redis 實例，並以 table 前綴 / Redis DB index 隔離資料。
...
│   ─ zutomayo db        │  │  Node HTTP · 13 個 service 模組  │
...
└────────────────────────┘  └─────────────────────────────────┘
                ...
                └────────── Redis 7 ────────┘
                   ...
                   matchmaking queue / rate limit / presence）
```
...
| 層          | 主要職責                                                                             | 技術                                                            |
| ----------- | ------------------------------------------------------------------------------------ | --------------------------------------------------------------- |
| 前端 SPA    | UI、路由、牌組編輯、教學、i18n、PWA、boardgame.io client（state sync + playerView）  | React 19、Vite 7、React Router 7、Tailwind/daisyUI、PWA         |
...
| API Server  | 帳號 / OAuth / 牌組 / 對戰結果 / 配對 / 反饋 / 管理 / 卡牌資料                       | Node HTTP、pg、ioredis、zod、node-pg-migrate                    |
| PostgreSQL  | 持久資料（用戶、牌組、對戰、卡牌、反饋、boardgame.io state）                         | PostgreSQL 16                                                   |
| Redis       | 跨節點同步、配對佇列、限流、presence                                                 | Redis 7（AOF + allkeys-lru）                                    |
...
入口：`api/server.cjs`（CommonJS，Node 原生 `http` server）。搭配 13 個 service 模組與 zod schema。
...
| `matchmakingService.cjs` | 配對佇列加入 / 狀態查詢 / 離開 / host 回報 boardgame.io matchID                |
...
Redis 同時扮演四種角色，全部透過 `REDIS_DB` 切到獨立 DB index（0-15）隔離：
...
| Matchmaking 佇列           | `mm:queue` (sorted set) / `mm:{userId}` (hash) | Lua 原子配對                                |
...
Lua 保證多 API 實例下不會把同一人配給兩人（原子 ZREM + HSET）。

### 建房 / 加入

- 配對成功後 **host** 用 boardgame.io lobby API `createMatch`（setupData 帶 `clientVersion`、雙方 deck 名/ids），取得 `matchID`。
- host 透過 `PUT /api/matchmaking/match` 回報 `realMatchId`，雙方 `mm:{userId}.realMatchId` 同步。
...
| `matchmaking_queue_depth`       | Gauge     | game | 當前配對佇列深度                                                       |
...
- **game / api 可多實例**：增加 replica 數即可。PG 與 Redis 維持單實例。
- **共享狀態**：
  - boardgame.io state → `PostgresAdapter` 寫 `bjg_matches`（stateID guard 防覆蓋）。
  - 跨節點廣播 → `RedisPubSub` + `@socket.io/redis-adapter`。
  - 配對佇列 / 限流 → Redis shared。
- **一致性要求**：所有同服務實例必須共享 `JWT_SECRET` 與 `ALLOWED_ORIGINS`，且 `APP_BUILD_ID` / `GAME_RULES_VERSION` 一致（否則 client 會被版本檢查擋下）。
...
| 配對 Lua          | `api/server.cjs`（`MATCH_LUA` / `CLEAN_LUA`）+ `api/matchmakingService.cjs`        |
