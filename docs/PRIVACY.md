...
- 遊戲資料：私人牌組、公開或僅連結的牌組分享快照、複製紀錄、配對、對局結果、ELO、伺服器產生的 action log、棄賽與制裁紀錄。
...
登入玩家可在個人頁匯出帳號、私人牌組、牌組分享、按讚、複製、檢舉、對局與社交資料，並可直接刪除帳號。匯出不包含其他玩家的私人資料或管理員內部備註。無法登入時，可由註冊 email 寄信至 `contact@mail.zutomayocard.online` 申請協助。

刪除時，email、登入身分、私人牌組、玩家擁有的牌組分享、按讚與社交關係會刪除；對他人分享的複製與檢舉紀錄會刪除或移除帳號識別。維持排名、反作弊與爭議處理所必需的比賽或審核資料，可在移除直接識別資訊後保留。法律、安全事件或爭議保存義務優先於一般刪除時程。
