...
| MA-04 | P0       | 將 stale cleanup 改成 archive-before-delete  | In progress | MA-03               | production 已封存 4 場 abandoned 且無 reconciliation mismatch；terminal/staging concurrency 證據待補                 |
...
| MA-07 | P1       | 建立資料品質檢查、metrics 與告警             | In progress | MA-03, MA-04        | production durable gauges 為 0；新增近 24h metadata 缺失告警；staging alert drill 待執行                             |
...
| MA-10 | P0       | Staging 驗證與 production rollout            | In progress | MA-03, MA-04, MA-07 | production 已部署並驗證 abandoned capture；dataset identity 修補、controlled terminal 與 24h reconciliation 待完成   |
...
| MA-RISK-09 | Runtime 未注入 release dataset identity     | 不同卡池資料無法安全分組分析 | production/staging fail closed、deployment smoke、24h 告警   | Open   |
