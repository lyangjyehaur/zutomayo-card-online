...
`traffic_class`、每席 disconnect／reconnect 計數、outstanding disconnect 時間與最多 100 筆連線摘要，foreign key
隨 `bjg_matches` 刪除。每筆摘要只保留座位、權威遊戲階段、開局後秒數與斷線秒數。`/resume` 只在首次
seat proof 之後再次成功恢復座位時增加 `bjg_match_seats.resume_count`。終局／abandoned projector 才把三組
固定兩席計數與再次 allowlist 的 connection events 移入永久層；不保存絕對連線時間、room、invite、
session、socket、user ID、IP 或自由文字。
...
| MA-03 | P0       | 在權威終局 transaction 寫入分析資料          | In progress | MA-02               | production 已永久保存 14 completed、2 surrendered；transaction/rollback/idempotency unit tests 已完成；draw/staging E2E 待執行         |
| MA-04 | P0       | 將 stale cleanup 改成 archive-before-delete  | In progress | MA-03               | production 新版已封存 7 個零 action 空殼，且 terminal/abandoned child reconciliation 均無 mismatch；staging concurrency 證據待補       |
| MA-05 | P1       | 保存配對來源與連線生命週期摘要               | In progress | MA-01, MA-02        | direct provenance、首次 proof 與 bounded WSS lifecycle 本地修補完成；待部署及 staging controlled fixtures                              |
| MA-06 | P1       | 建立匿名 replay projector 與永久保存政策     | In progress | MA-01, MA-02        | production 16 個 terminal facts 的 753 個 typed events 全數 reconciliation；staging 原局重算 review 待完成                             |
| MA-07 | P1       | 建立資料品質檢查、metrics 與告警             | In progress | MA-03, MA-04        | capture counter 改為 COMMIT 後計數並有 commit-failure tests；待部署 reconciliation 與 staging alert drill                              |
| MA-08 | P1       | 建立核心分析 queries 與最低樣本規則          | In progress | MA-03, MA-05, MA-06 | 空房 cohort 與 lifecycle step/gap query/PG fixture 已本地驗證；production 最高 deck appearance 12，未達 100 門檻                       |
| MA-09 | P1       | 評估歷史資料回填與不可恢復範圍               | Open        | MA-01               | isolated restore report、backfill manifest                                                                                             |
| MA-10 | P0       | Staging 驗證與 production rollout            | In progress | MA-03, MA-04, MA-07 | production identity、自然 terminal capture 與 child reconciliation 已驗證；controlled draw/replay review、24h/7d reconciliation 待完成 |
...
1. `match start -> completed/abandoned`，按 mode、seat class、版本與日期分組。
   `missing-seat-reservation` 且零 action 的未成局空房必須獨立成 cohort，不得計入已開打對局的 abandonment rate。
...
| MA-RISK-10 | 未成局空房被計入 gameplay abandonment        | 完成率與中離率被系統性低估   | 依 seat/action quality 分 cohort；修改 query 與 expected data | Open      |
| MA-RISK-11 | 首次入場 proof 被 `resume_count` 計為 resume | 重連與恢復率被系統性高估     | 分離 initial proof 與真正 resume；修補 SQL/標記並補 E2E       | Open      |
| MA-RISK-12 | Direct 對戰缺 provenance 且重連訊號偏高      | 無法可靠歸因連線穩定性問題   | 補 server-owned direct provenance；分 mode 追蹤 WSS lifecycle | Open      |
| MA-RISK-13 | Capture counter 在 transaction COMMIT 前增加 | rollback 被誤報為永久保存    | COMMIT 後才計數；terminal/abandoned commit-failure tests      | Mitigated |
...
| MA-EV-20260801-08 | 2026-08-01 | MA-03/04/05/06/07/08/10 | Production `0.2.6` / SHA `fec9501…` / dataset `7bad73a4…5770`                           | 同一部署後區間追加唯讀 aggregate：23 facts 與 game process capture counter 完全一致（14 completed、2 surrendered、7 abandoned），46/46 deck rows、753/753 event rows、build/dataset/environment/integrity 全數 reconciliation。16 個 terminal facts 含 8 custom_room、8 direct；時長 p10/p50/p90 為 79.5/184/318.5 秒，回合為 3/4/6.5。Direct terminal 中 4/8 有 disconnect+reconnect，自訂房為 0/8；7 個 abandoned 全部零 action、缺一席且無 events，證明是未成局空房，不能當成 gameplay abandonment。16/16 terminal 都有 seat resume，顯示初次 seat proof 與真正 resume 語義混用。Seat 0 為 12/16（75%，Wilson 95% 50.5–89.8%），猜拳勝者為 7/16（43.8%，23.1–66.8%）；9 個 distinct decks 中最高只出現 12 次，所有 balance 結論皆未達 100 appearance publication gate。 | production read-only aggregate inspection；僅保存彙總，未記錄 match/user ID | Durable terminal verified; follow-up required |
| MA-EV-20260801-09 | 2026-08-01 | MA-05/07/08             | Branch `codex/match-analytics-post-deploy-evidence`                                     | 依首批 production 證據修正兩個資料語義：daily funnel 將零 action 且缺 seat 的 abandoned shell 列為 `unformed_sessions`，排除於 gameplay completion/abandonment denominator；`resume_count` 首次 seat proof 只設定時間，後續 proof 才累加。聚焦 16 tests、fresh PostgreSQL role/schema/fixture expected-result smoke 與 `npm run verify` 通過（217 files／1,680 tests、coverage、production build）。                                                                                                                                                                                                                                                                                                                                                                       | repository branch；尚未部署                                                 | Local verified; deploy pending                |
| MA-EV-20260801-10 | 2026-08-01 | MA-02/03/05/07/08       | Branch `codex/match-analytics-post-deploy-evidence` / migration `000050`                | 修正 trusted direct provenance；新增最多 100 筆、不含身份的 disconnect/reconnect step、offset 與 gap 摘要並投影為永久 typed events；新增 lifecycle aggregate p50/p90 query。Capture counter 改為 transaction COMMIT 成功後才增加，terminal 與 abandoned commit failure 均驗證不誤計。聚焦 65 tests、fresh PostgreSQL migration/schema/role/fixture smoke 與 `npm run verify` 通過（217 files／1,683 tests、coverage、production build）。                                                                                                                                                                                                                                                                                                                                  | repository branch；尚未推送或部署                                           | Local verified; deploy pending                |
...
| 2026-08-01 | 分析首批 production 自然終局   | 23 個新版 facts 與 process counters、deck/event children、release identity、integrity 全數一致，永久封存已取得自然 completed/surrendered 證據；同時發現空房污染 abandonment、初次 proof 污染 resume，以及 direct 4/8 terminal 發生重連。16 個 terminal 不足以判定 seat/janken/deck balance | 先修 MA-08 cohort query 與 MA-05 resume/provenance 語義，再以 staging controlled fixtures 驗證；production 持續累積至 24h/7d 與 100 appearance 門檻，direct reconnect 以 WSS lifecycle 指標追因            | Production review    |
| 2026-08-01 | 修正首批 production 資料語義   | Query 已分離未成局空房與 gameplay abandonment；首次 seat proof 不再增加 resume。Disposable PG expected result 與全量 verify 通過；既有永久 evidence 維持原值，不改寫歷史資料                                                                                                               | 建立 follow-up PR；部署後驗證新 build 的 seat resume baseline 為 0，並為 direct 對戰補可歸因的 WSS lifecycle 指標與 provenance                                                                             | Codex implementation |
| 2026-08-01 | 補齊連線追因與 durable metric  | Trusted direct 不再誤標缺來源；runtime 只保存 bounded step/offset/gap 並以 typed events 永久化；capture counter 移到 COMMIT 後，rollback 不再造成假成功                                                                                                                                    | 與其他任務合併部署；部署後唯讀核對 counter/facts、direct quality flags、resume baseline、connection event reconciliation，持續 24h/7d aggregate 觀察                                                       | Codex implementation |
