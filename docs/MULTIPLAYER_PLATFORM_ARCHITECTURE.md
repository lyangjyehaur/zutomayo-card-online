...
Custom-room pregame chat uses the durable `ChatService` room conversation type keyed by the room code. The lobby can sync, send, translate, report, and mark room chat read before the boardgame match starts; the host waiting page registers a Colyseus `custom_room` in `waiting` status for realtime discovery/membership, the first non-host join promotes it to `ready` and relays the boardgame match ID, and Colyseus still does not own chat persistence. If the Colyseus custom-room relay times out, disconnects, or is cancelled, the browser surfaces a retryable room-join error instead of falling back to a direct boardgame.io room-code join.
