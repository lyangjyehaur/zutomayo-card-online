...
Chat translation is ChatService-owned and provider-pluggable. When `CHAT_TRANSLATION_ENDPOINT` is configured, the API calls that HTTP LLM gateway and persists ready translations; when no provider is configured, translation requests are still recorded as pending rows so the UI and future asynchronous workers share the same durable contract.
