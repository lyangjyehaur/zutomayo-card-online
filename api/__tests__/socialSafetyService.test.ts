...
describe('social safety service', () => {
  it('creates a pending friend request without granting friendship', async () => {
    ...
    await expect(
      createFriendRequest({ pool, userId: 'u_me', targetUserId: 'u_friend' }),
    ).resolves.toMatchObject({ ok: true, body: { friendUserId: 'u_friend', request: { status: 'pending' } } });
  ...
  it('only lets the recipient accept and creates friendship in both directions', async () => {
    ...
    await expect(
      respondToFriendRequest({ pool, userId: 'u_me', requestId: 9, accept: true }),
    ).resolves.toEqual({ ok: true, body: { accepted: true } });
    expect(pool.query).toHaveBeenCalledWith(expect.stringContaining('VALUES ($1, $2), ($2, $1)'), [
      'u_sender',
      'u_me',
    ]);
  });
